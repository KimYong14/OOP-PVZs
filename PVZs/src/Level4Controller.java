...
public class Level4Controller implements Serializable {
    ...
    @FXML
    protected Button cherrybomb_button;
    ...
    @FXML
    protected void cherrybomb_selection(ActionEvent event) throws IOException {
        this.plant_to_place = "cherrybomb";
        this.plantSelected = true;
    }
...
//            System.out.println("everyone rox");
            ...
            if(this.plantSelected==true && arr[colIndex.intValue()][rowIndex.intValue()]==false)
            {
                ...
                if (this.plant_to_place=="sunflower")
                {
...
//                    System.out.println( Lanes[colIndex.intValue()][rowIndex.intValue()] );
                ...
                if (this.plant_to_place=="peashooter")
                {
...
//                    System.out.println( Lanes[colIndex.intValue()][rowIndex.intValue()] );
                ...
                if (this.plant_to_place=="cherrybomb")
                {
                    CherryBomb c = new CherryBomb(my_gridPane);
                    GridPane.setConstraints(c, colIndex.intValue(), rowIndex.intValue()); // column=2 row=0
                    my_gridPane.getChildren().addAll(c);
                    arr[colIndex.intValue()][rowIndex.intValue()]=true;
                    Lanes[colIndex.intValue()][rowIndex.intValue()]=c;
//                    System.out.println("vals "+colIndex.intValue()+" "+ rowIndex.intValue() );
//                    System.out.println( Lanes[colIndex.intValue()][rowIndex.intValue()] );

                    cherrybomb_button.setDisable(true);

                    final Timeline animation = new Timeline(
                            new KeyFrame(Duration.seconds(7),
                                    new EventHandler<ActionEvent>() {
                                        @Override public void handle(ActionEvent actionEvent) {
                                            cherrybomb_button.setDisable(false);
                                        }
                                    }));
                    animation.setCycleCount(1);
                    animation.play();
